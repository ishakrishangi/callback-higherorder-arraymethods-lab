...
// CODE HERE
const evenNumbers  = mixedNumbers.filter(numbersArr => numbersArr % 2 === 0)
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(pocketMonst => pocketMonst.CP > 200)
...
const taxedOrders = orders.map(priceTax => (1 + priceTax.tax) * priceTax.price)
...
const bobsTotal = purchases.filter(bobsPurchases => bobsPurchases.owner === "Bob").reduce((a, b) => a + b.price, 0)
const barrysTotal = purchases.filter(barrysPurchases => barrysPurchases.owner === "Barry").reduce( (a,c) => a + c.price, 0)

console.log(bobsTotal)
console.log(barrysTotal)